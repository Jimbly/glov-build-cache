...
This task wrapper is for caching single-input tasks such as image minification or other image postprocessing.  Tasks that output multiple outputs (such as a .wav -> .ogg + .mp3 conversion task) are also supported, as long as they are of type `gb.SINGLE`.  Any job will be re-run if the hash of the single input file has changed, or the task version has changed (no hashing of the child task's func, or it's version number, etc, is done).
